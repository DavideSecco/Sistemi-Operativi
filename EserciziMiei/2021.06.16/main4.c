...
int main(int argc, char *argv[]){
	...
	// creo il file con il mio cognome:
	if ((fcreato = creat("Secco", O_WRONLY)) < 0) { 
		printf("Errore nella creazione del file\n");
		exit(-1);
	}

	printf("sizeof(tutteLinee) = %d\n", sizeof(tutteLinee));
	printf("sizeof(lin) = %d\n", sizeof(lin));

	//ALLOCAZIONE MEMORIA MALLOC

	// alloco per l'array che si deve inviare fra figli e padre:
	if((tutteLinee = malloc(N*sizeof(linea)))== NULL){
		printf("Errore allocazione dell'array\n");
    	exit(3); 
	}
    ...
    for (i=0; i < N; i++) {
		...
		if (pid == 0) /* figlio */ {
			...
			for(j = 0; ; j++){
				...
				while(nr = (read(fd, &lin[k], 1))){
					if(lin[k] == '\n'){
						lin[k+1] = '\0';

						read(piped[i][0], tutteLinee, sizeof(tutteLinee)*sizeof(lin));

						printf("FIglio %d riga %d: %s", i, j, lin);

						strcpy(tutteLinee[i], lin);

						write(piped[(i+1)%N][1], tutteLinee, sizeof(tutteLinee)*sizeof(lin));
				...
				/*if(i == (N-1)){
					write(fcreato, tutteLinee, sizeof(tutteLinee)*sizeof(lin));
				}*/
	...
	write(piped[0][1], tutteLinee, sizeof(tutteLinee)*sizeof(lin));		// lancio l'innesco
	// sleep(1);
